/*
Simple script to move my tiny 6dof robotic arm   Cursor Edit
*/
#include <math.h>
...
double curPos1 = 0.0;     double HomePos1 = 0.0;    double StartPos1 = 0.0;
double curPos2 = -78.51;  double HomePos2 = -78.51; double StartPos2 = 0.0;
double curPos3 = 73.90;   double HomePos3 = 73.90;  double StartPos3 = 0.0;
double curPos4 = 0.0;     double HomePos4 = 0.0;    double StartPos4 = 0.0;
double curPos5 = -90.0;     double HomePos5 = -93.0;    double StartPos5 = 90.0;
double curPos6 = 0.0;     double HomePos6 = 0.0;    double StartPos6 = 0.0;
...
const double dl2 = 360.0/200.0/32.0/4.5;
...
// Web Serial (Chrome) genelde USB CDC = Serial, 115200 baud. Bluetooth = Serial1.
#define USB_BAUD 9600
#define BT_BAUD 9600
/** readString() satır sonu beklemeden hatalı birleştirir; simülatör \n göndermeyebilir. */
#define CMD_IDLE_END_MS 45
...
/**
 * Web Serial genelde satır sonu göndermez; komutlar arası kısa sessizlikte paketi kapatır.
 * \r veya \n gelirse komut hemen biter.
 */
bool readOneCommand(Stream &port, String &out) {
  ...
  while (true) {
    ...
    if (out.length() > 0 && (millis() - lastRead) >= CMD_IDLE_END_MS) {
      out.trim();
      return out.length() > 0;
    }
...
void handleSerialCommand(String cmd) {
  ...
  // Hız — mutlaka s1..s6 önce (ss ile çakışma olmaz; yine de net sıra)
  if (cmd.startsWith("ss")) {
    String dataInS = cmd.substring(2, cmd.length());
    curSpeed = (dataInS.toFloat() / 100.0) * 0.5e-4;
    curFinalSpeed = (dataInS.toFloat() / 100.0) * 0.5e-4;
    Serial.println("Speed: " + dataInS);
  }

  if (cmd.startsWith("s6")) {
    String dataInS = cmd.substring(2, cmd.length());
  ...
  if (cmd.startsWith("save")) {
    String dataInS = cmd.substring(4, cmd.length());
    Serial.println(dataInS+",P1"+curPos1+",P2"+curPos2+",P3"+curPos3+",P4"+curPos4+",P5"+curPos5+",P6"+curPos6);
    index++;
    InSpeed[index] = curFinalSpeed;
    curFinalSpeed = (dataInS.toFloat()/100)*0.5e-4;
    Joint1[index] = curPos1;
  ...
  if (cmd.startsWith("move1")) {
    String dataX = "";
    Serial.println("Hareket 1 ");
    doEnable();
    goStartPos();
    delay(500);
    Serial.println(dataX + "P1:" +curPos1 + ",P2:" + curPos2 + ",P3:" + curPos3+ ",P4:" + curPos4 + ",P5:" + curPos5 + ",P6:" + curPos6);
    delay(500);
    gogogo();
    delay(1000);
    Serial.println(dataX + "P1:" +curPos1 + ",P2:" + curPos2 + ",P3:" + curPos3+ ",P4:" + curPos4 + ",P5:" + curPos5 + ",P6:" + curPos6);
    Serial.println(dataX + "HP1:" +HomePos1 + ",HP2:" + HomePos2 + ",HP3:" + HomePos3+ ",HP4:" + HomePos4 + ",HP5:" + HomePos5 + ",HP6:" + HomePos6);
  }

  if (cmd.startsWith("move2")) {
    Serial.println("Hareket 2 ");
    doEnable();
    goStartPos();
    goMove3();
    goStartPos();
  ...
  if (cmd.startsWith("simcom")) {
    Serial.println("Simulator Eklem Hareketi");
    //simcom11.00,2.00,3.00,4.00,5.00,6.00
    //String dataInS = cmd.substring(7, cmd.substring .length());
    Serial.println(cmd);
    parseSimJointS(cmd);    
    Serial.println(String(futPos1) + "," + futPos2 + "," + futPos3 + "," + futPos3 + "," + futPos4 + "," + futPos5);
    if (IsEnable == false) doEnable();    
    goSimJoints();
  ...
  if (cmd.startsWith("run")) {
    Serial.println("Run");
    goStartPos();
    Joint1[0] = curPos1; Joint2[0] = curPos2; Joint3[0] = curPos3; Joint4[0] = curPos4; Joint5[0] = curPos5; Joint6[0] = curPos6;

    Serial.println(String("StartPos > ") + "P1:" +curPos1 + ",P2:" + curPos2 + ",P3:" + curPos3+ ",P4:" + curPos4 + ",P5:" + curPos5 + ",P6:" + curPos6);
    delay(500);
    ...
    for (int i = 0; i <= index-1; i++) {
      ...
      Serial.println(String("Jinitial[") + i + "] > P1: " + Joint1[i] + ",P2: " + Joint2[i] + ",P3: " + Joint3[i]+ ",P4: " + Joint4[i] + ",P5: " + Joint5[i]+ ",P6: " + Joint6[i]);
      Serial.println(String("Jfinal{") + (i+1) +  "]> P1: " + Joint1[i+1] + ",P2: " + Joint2[i+1] + ",P3: " + Joint3[i+1]+ ",P4: " + Joint4[i+1] + ",P5: " + Joint5[i+1]+ ",P6: " + Joint6[i+1]);
      delay(100);
    }
    delay(500);
    goStartPos();
...
void parseSimJointS(String str) {
  
  // 1. "simcom" komutunu al (ilk 6 karakter)
  String dataInS = str.substring(0, 6);

  // 2. Sayıların başladığı yeri bul
  int startIndex = 6; // ilk sayının başlangıcı

  if (startIndex == -1) return;

  String numbers = str.substring(startIndex);

  // 3. Virgüllere göre parçala
  float values[6];
  int index = 0;
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (commaIndex != -1) {
      values[i] = numbers.substring(0, commaIndex).toFloat();
      numbers = numbers.substring(commaIndex + 1);
    } else {
      values[i] = numbers.toFloat(); // son değer
    }
  }
...
{
  String cmd;
  if (readOneCommand(Serial, cmd)) {
    handleSerialCommand(cmd);
  }
  if (readOneCommand(Serial1, cmd)) {
    handleSerialCommand(cmd);
  }
}
...
{
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={futPos1, futPos2, futPos3,futPos4, futPos5, futPos6};  
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);
}
void goMove2()
{
  Serial.println("goMove2_0");
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={curPos1, 45, 35,curPos4, -90, curPos6};  
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);  
  Serial.println("goMove2_1");
  goMove2_1();
  delay(500);
  Serial.println("goMove2_2");
  goMove2_2();
  delay(500);
  Serial.println("goMove2_3");
  goMove2_3();
  delay(500);
  Serial.println("goMove2_4");
  goMove2_4();  
}
...
{
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={curPos1, 55, 0,curPos4, -60, curPos6};  
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);
}
...
{
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={-30, 50, 15,35, -70, -15};  
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);
...
{
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={0, 45, 35,0, -90, curPos6};  
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);
}
...
void goTouch()
{
  // dokunma hareketi  
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={-5, curPos2, curPos3, curPos4, 80, curPos6};  
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);
}

void goMove3()
{
  float Jinitial1[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal1[6]={0, 20, 3.34,0, 70, 0};  
  goStrightLine(Jinitial1, Jfinal1, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100); 
  
  float Jinitial2[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal2[6]={18, 20, 12.5,40, 70, 0};  
  goStrightLine(Jinitial2, Jfinal2, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100);

  float Jinitial11[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal11[6]={0, 20, 3.34,0, 70, 0};  
  goStrightLine(Jinitial11, Jfinal11, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100); 
  
  float Jinitial3[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal3[6]={-14, 20, 12,-40, 70, 0};  
  goStrightLine(Jinitial3, Jfinal3, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100);
  
  float Jinitial4[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal4[6]={0, 20, 3,0, 70, 0};  
  goStrightLine(Jinitial4, Jfinal4, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100);

  float Jinitial5[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal5[6]={0, 50, -30,0, 115, 0};  
  goStrightLine(Jinitial5, Jfinal5, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100);
  float Jinitial55[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal55[6]={0, 20, 3.34,0, 70, 0};  
  goStrightLine(Jinitial55, Jfinal55, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100); 
  float Jinitial6[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal6[6]={0, -9, 42,0, 22, 0};  
  goStrightLine(Jinitial6, Jfinal6, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100);
  float Jinitial66[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal66[6]={0, 20, 3.34,0, 70, 0};  
  goStrightLine(Jinitial66, Jfinal66, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(100); 
...
void goTouchBack()
{
  // dokunma hareketi  
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={0, curPos2, curPos3, curPos4, 85, curPos6};  
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);
}
...
{
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={HomePos1, HomePos2, HomePos3, HomePos4, HomePos5, HomePos6};  
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);
  delay(500);
  doDisable();
...
{
  float Jinitial[6]={curPos1, curPos2, curPos3, curPos4, curPos5, curPos6};
  float Jfinal[6]={StartPos1, StartPos2, StartPos3, StartPos4, StartPos5, StartPos6};
  goStrightLine(Jinitial, Jfinal, curSpeed, 0.75e-10, 0.0, 0.0);
}
...
void gogogo()
{
  ...
  */
  ...
  goStrightLine(Jhome, J1, velG, accG,0.0, 0.0);  
  goStrightLine(J1, J11, velG, accG, 0.0, 0.0);
  goStrightLine(J11, J12, velG, accG, 0.0, 0.0);
  goStrightLine(J12, J13, velG, accG, 0.0, 0.0);
  goStrightLine(J13, J14, velG, accG, 0.0, 0.0);
  goStrightLine(J14, J15, velG, accG, 0.0, 0.0);
  goStrightLine(J15, J16, velG, accG, 0.0, 0.0);
  goStrightLine(J16, J17, velG, accG, 0.0, 0.0);
  goStrightLine(J17, J1, velG, accG, 0.0, 0.0);

  goStrightLine(J1, J18, velG, accG, 0.0, 0.0);
  goStrightLine(J18, J14, velG, accG, 0.0, 0.0);
  goStrightLine(J14, J1, velG, accG, 0.0, 0.0);
  
  goStrightLine(J1, J2, velG, accG, 0.0, 0.0);
  goStrightLine(J2, J1, velG, accG, 0.0, 0.0);

  goStrightLine(J1, J3, velG, accG, 0.0, 0.0);
  goStrightLine(J3, J1, velG, accG, 0.0, 0.0);

  goStrightLine(J1, J4, velG, accG, 0.0, 0.0);
  goStrightLine(J4, J1, velG, accG, 0.0, 0.0);
  
  goStrightLine(J1, Jhome, velG, accG, 0.0, 0.0);
}
